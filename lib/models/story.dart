...
class Story{
  ...
  Map<String, Object> toDBValue(){
    return {
      ...
      'title' : title,
      'by' : by,
      'time' : time.toIso8601String(),    // Because time is stored as string in the database.
      'url' : url
    };
  }
 ...
 // Optional fields omitted due to reason stated above.
  factory Story.fromJson(Map<String, dynamic> storyJson){
    return Story(
      ...
      url: storyJson["url"]??storyJson["text"]??"https://www.butter.us/", // Due to inconsistent data recieved from the API on some calls.
    );
  }
